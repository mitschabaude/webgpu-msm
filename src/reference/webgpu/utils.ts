...
export const generateRandomScalars = (n: number): bigint[] => {
  const scalars = [];
  for (let i = 0; i < n; i++) scalars[i] = createRandomAleoScalarInt();
  return scalars;
};
...
export const bigIntsToBufferLE = (
  bigInts: bigint[],
  bigIntSize = 256
): Buffer => {
  const bigIntBuffers = bigInts.map((bigInt) =>
    bigIntBufferLE(bigInt, bigIntSize)
  );
...
export const readBigIntsFromBufferLE = (
  ...
  for (let i = 0; i < totalBigInts; i++) {
    ...
    const singleBigIntBuffer = buffer.slice(
      i * (bigIntSize / 8),
      (i + 1) * (bigIntSize / 8)
    );
...
const createRandomAleoScalarInt = () => {
  let bigIntString = "";
  for (let i = 0; i < 8; i++) {
    bigIntString += Math.floor(Math.random() * (2 ** 32 - 1));
  }
  return BigInt(bigIntString) % SUBGROUP_CHARACTERISTIC;
};
